...
\begin{document}
	...
	\author{\IEEEauthorblockN{Yue Dong}
		...
		The building of this model is counter-intuitive because for an ensemble of classifiers to be more accurate than any of the individual members, it is necessary and sufficient that the classifiers are accurate and diverse \cite{Hansen}. An accurate classifier is one that has an error rate of better than 50\%. Two classifiers are diverse if they don't make errors on the same new data points \cite{Dietterich}. We therefore believe that there is a tradeoff between accuracy of individual models and diversity of the ensemble as a whole. In particular, in the case where the weights matrix is passed between subsequent neural network models, it can be considered equivalent to having trained the neural network on more data points, which increases the accuracy of a single classifier in the ensemble. On the other hand, since the classifiers are built from similar data with related weight we lose the diversity.  
		
		However, the diversity issue can be solved by careful models selection. Since the size of ensembles is fixed, some The models picking means we only pick a constant $K$ classifiers to be in the ensemble. With the principle that we only pick classifiers who are diverse enough to each other, we can have the classifiers in the ensemble more accurate and diverse.
		...
		This section describes what happens once the ensemble has reached its maximum
		size $C$. Recall that since memory is limited, we cannot keep adding models indefinitely.
		However, since the ensemble may not be optimal yet at the time that it reaches size $C$, we do
		not want to discard all further models either. Indeed, we want the models of the ensemble to be as \emph{diverse} as possible and this require ongoing evaluation of potential new models.
		...
		* The current model picking methods are:
		...
		\begin{enumerate}
			...
			\item RRGD - Random Replacement with Geometric Distribution
			\item MDS  - Maximal Diversity Subset
		\end{enumerate}

		* Moreover, it has all the usual underlying parameters of neural networks:
		...
		\subsection{Diversity of Ensembles}
		
		As mentioned in Section~\ref{sec:senn}, there is an important tradeoff between accuracy and diversity
